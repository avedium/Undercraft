...
vec3 lightDay        = vec3(LIGHT_DAY_RED,   LIGHT_DAY_GREEN,   LIGHT_DAY_BLUE)   * LIGHT_DI / 255.0;
vec3 lightEvening    = vec3(LIGHT_ER,   LIGHT_EG,   LIGHT_EB)   * LIGHT_EI / 255.0;
vec3 lightNight      = vec3(LIGHT_NIGHT_RED,   LIGHT_NIGHT_GREEN,   LIGHT_NIGHT_BLUE)   * LIGHT_NI * 0.3 / 255.0;

vec3 ambientMorning  = vec3(AMBIENT_MR, AMBIENT_MG, AMBIENT_MB) * AMBIENT_MI / 255.0;
vec3 ambientDay      = vec3(AMBIENT_DAYR, AMBIENT_DAYG, AMBIENT_DAYB) * AMBIENT_DAYI / 255.0;
vec3 ambientEvening  = vec3(AMBIENT_ER, AMBIENT_EG, AMBIENT_EB) * AMBIENT_EI / 255.0;
vec3 ambientNight    = vec3(AMBIENT_NIGHTR, AMBIENT_NIGHTG, AMBIENT_NIGHTB) * AMBIENT_NIGHTI * 0.3 / 255.0;
