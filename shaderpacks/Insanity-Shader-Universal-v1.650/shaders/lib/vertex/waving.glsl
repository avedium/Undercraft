...
vec3 CalcMove(vec3 pos, float density, float speed, vec2 mult) {
    pos = pos * density + vec3(0.0, frametime * (speed*6), 0.0);
    vec3 wave = vec3(Noise2D(pos.yz + 0.667), Noise2D(pos.xz - 0.633), Noise2D(pos.xy));
...
vec3 WavingBlocks(vec3 position, float istopv) {
    ...
    if (mc_Entity.x == 10100 && istopv > 0.9)
        wave += CalcMove(worldpos, 1.35, 1.0, vec2(0.25, 0.06));
    if (mc_Entity.x == 10101 && (istopv > 0.9|| fract(worldpos.y + 0.005) > 0.01))
        wave += CalcMove(worldpos, 1.7, 1.35, vec2(0.12, 0.06));
    if (mc_Entity.x == 10102 && (istopv > 0.9 || fract(worldpos.y + 0.005) > 0.01) ||
        mc_Entity.x == 10103)
        wave += CalcMove(worldpos, 1.35, 1.15, vec2(0.15, 0.06));
    if (mc_Entity.x == 10104 && (istopv > 0.9 || fract(worldpos.y + 0.0675) > 0.01))
        wave += CalcMove(worldpos, 1.35, 1.0, vec2(0.15, 0.06));
    if (mc_Entity.x == 10106)
        wave += CalcMove(worldpos, 1.35, 1.25, vec2(0.06, 0.06));        
    ...
    if (mc_Entity.x == 10105)
        wave += CalcMove(worldpos, 0.25, 1.0, vec2(0.28, 0.18));
    #endif
    #ifdef WAVING_LIQUID
