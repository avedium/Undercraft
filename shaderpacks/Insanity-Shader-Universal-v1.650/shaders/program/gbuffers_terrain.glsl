...
void main() {
	...
	if (albedo.a > 0.001) {
		...
		float metalness      = 0.0;
		float emission       = (emissive + candle) * 0.4 + lava;
		...
		float handlight = clamp((heldLightValue - 1.4 * length(viewPos)) / 15.0, 0.0, 0.9333);
		...
		if (recolor > 0.5) {
			albedo.rgb = blocklightCol * pow(ec, 1.5) / (BLOCKLIGHT_INTENSITY * BLOCKLIGHT_INTENSITY);
			albedo.rgb /= 0.7 * albedo.rgb + 0.7;
		}
		if (lava > 0.5) {
			albedo.rgb = pow(blocklightCol * ec / BLOCKLIGHT_INTENSITY, vec3(2.0));
